...
class reservationC{
    ...
    public function ajouterReservation($Reservation, $qty, $idservice, $user)
    {
        ...
        $sql = "INSERT INTO reservation ( idreservation,firstname, lastname, adresse,tel,email,nbn,date,service,rp,idservice,iduser)
			VALUES (:idreservation,:firstname,:lastname,:adresse,:tel,:email,:nbn,:date,:service,:rp,:idservice,:iduser)";
    ...
    function supprimerReservation($idreservation, $idservice)
    {
        ...
        $req1->bindValue(':idservice', $idroom);
        try {
            $req->execute();
            $req1->execute();
        } catch (Exception $e) {
    ...
    function updateReservation($Reservation, $idreservation)
    {
        try {
            ...
            $db = config::getConnexion();
            $query = $db->prepare(
                "UPDATE reservation SET idreservation=:idreservation, firstname=:firstname, lastname=:lastname, adresse=:adresse,tel=:tel,email=:email,nbn:=nbn,date=:date,service=:service,rp=:rp ,idservice=:idservice,iduser=:iduser WHERE idreservation = :idreservation"
            );
            echo "pass";
            echo
