...
class UserCore {
	...
	function afficherUsersExceptMe($id){
		...
		$sql="SELECT u.id, u.nom, u.prenom, u.Login, u.role, count(m.id) FROM utilisateur u LEFT JOIN message m ON u.id = m.source and m.destinataire=".$id." and m.readDestination=0 where u.id!=".$id." and u.role='admin' group by (u.id) ";
	...
	function afficherPatientExceptMe($id){
		...
		$sql="SELECT u.id, u.nom, u.prenom, u.Login, u.role, count(m.id) FROM utilisateur u LEFT JOIN message m ON u.id = m.source and m.destinataire=".$id." and m.readDestination=0 where u.id!=".$id." and u.role='Patient' group by (u.id) ";
	...
	function logIn($username, $password) {
		...
		foreach ($liste as $raw) {
			if($raw['Login'] == $username && $raw['Password'] == $password) {
				return $raw;
			}
		}
		return NULL;
